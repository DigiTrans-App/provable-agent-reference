...
## Local safety and recovery controls

- Reset and seed commands are development-only administrative operations. They require an
  explicit local-profile marker, refuse non-local endpoints and non-synthetic tenant data, print
  the resolved targets, and fail closed when environment identity is missing or ambiguous.
- Reset tooling cannot share credentials or network reachability with a non-synthetic deployment.
- Artifact publication follows staged content-addressed upload, digest verification, durable
  outbox finalization, and orphan reconciliation; database state never implies that an artifact
  exists until finalization is recorded.
- Journal append, state transition, and outbox enqueue share one database transaction. Normal
  application roles have no journal update or delete permission.
