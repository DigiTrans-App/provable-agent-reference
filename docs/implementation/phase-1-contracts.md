...
- 18 schema-negative mutations;
- 11 schema-valid semantic failures.
...
- compensation execution and durable idempotency-state enforcement;
...
The clean PR A revision adds executable checks for acknowledgement-versus-effect semantics,
timestamp/status consistency, first-link and contiguous sequence handling, delegation
attenuation, idempotency input rebinding, unsafe retry after an unknown effect, and
self-issued conformance provenance. Canonical body and record hash recomputation remains gated
on the separately specified canonical hash-input contract and must be implemented before an
authenticated-record capability can pass.
