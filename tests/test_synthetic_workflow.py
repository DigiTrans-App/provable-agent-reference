...
class SyntheticWorkflowTests(unittest.TestCase):
    ...
    def test_workflow_integrates_minimized_adapters_with_existing_control_chain(self):
        ...
        self.assertEqual(result.activities[0]["result_count"], 1)
        self.assertNotIn("text", result.activities[0]["results"][0])
        self.assertEqual(result.activities[0]["minimization_decision"], "minimized")
        self.assertEqual(result.activities[1]["result_status"], "succeeded")
